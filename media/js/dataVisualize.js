  ...
  function parseCSV(csvUrl)
  {
	...
	$.get(csvUrl, function(data) {
		...
		for(i = 1; i < csvData.length; i++)
		{
			...
			for( areaName in areaNamesToNumbers)
			{
				...
				tempDataValue = tempDataValue.replace(",", "");
				var tempDataNumber = parseFloat(tempDataValue);
				//make sure we have a real number
				if(isNaN(tempDataNumber) || tempDataNumber == Number.NEGATIVE_INFINITY || tempDataNumber == Number.POSITIVE_INFINITY)
				{
					continue;
				}
				areaData[areaName] = tempDataNumber;
	...
	function showByIndicator(indicator)
	{
		...
		{				
			...
			if(indicatorsToUpdateParams[indicator]["source"] == "")
			{
			...
			else
			{
				...
				if(indicatorsToUpdateParams[indicator]["link"] == "")
				{
					//not a valid link
					$("#sourceURL").hide();
					$("#sourceNoURL").show();
					$("#sourceNoURL").text(indicatorsToUpdateParams[indicator]["source"]);
				}
				else
				{
					$("#sourceNoURL").hide();
